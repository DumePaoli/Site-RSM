...
export const adminOrders = (page = 1, search = '', status = '') => api.get(`/api/admin/orders?page=${page}&search=${encodeURIComponent(search)}&status=${status}`).then(r => r.data)
export const adminExportCSV = () => window.open('/api/admin/orders/export-csv', '_blank')
...
export const adminDeleteManualLicense = (key) => api.delete(`/api/admin/manual-licenses/${key}`).then(r => r.data)
...
export const adminDeleteKeyPermanent = (key) => api.delete(`/api/admin/keys/${key}/permanent`).then(r => r.data)
export const adminClearActivations = (key) => api.delete(`/api/admin/keys/${key}/activations`).then(r => r.data)
export const adminRefreshVersion = () => api.post('/api/admin/refresh-version').then(r => r.data)
